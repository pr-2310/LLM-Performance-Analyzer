    ...
    <div class="main_graph" ref="graphContainer">
        ...
        <div class="float-search-window">
            <input type="text" v-model.lazy="searchText" placeholder="Search"
                style="width: 200px; padding: 10px; border-radius: 20px" />
            <div>
        ...
        <div class="float-node-info-window">
            ...
            <div v-for="(value, key) in all_node_info[selected_node_id]" :key="key" class="float-node-info-item">
                <span v-if="['bound'].includes(key)"><strong>{{ key }} :</strong> {{ value }}</span>
                <span v-else-if="['inference_time'].includes(key)"><strong>{{ key }} :</strong> {{ strNumberTime(value)
                    }}</span>
                <span v-else><strong>{{ key }} :</strong> {{ strNumber(value) }}</span>
            </div>
...
function graphUpdate() {
    ...
    axios
        ...
        .then(function (response) {
            ...
            const is_equal =
                old_ids.size === new_ids.size &&
                [...old_ids].every((key) => new_ids.has(key));
...
function update_roofline_model() {
    ...
    if (ctx) {
        ...
        if (selected_node_id.value) {
            const node_arithmetic_intensity =
                all_node_info.value[selected_node_id.value]["arithmetic_intensity"];
